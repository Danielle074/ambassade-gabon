<template>
  <div class="min-h-screen bg-gray-50">
    <!-- Hero section -->
    <div class="relative bg-gradient-to-r from-[#3A75C4] to-[#009E60] text-white">
      ...
      <div class="relative max-w-7xl mx-auto px-4 py-16 md:py-20">
        <div class="text-center">
          ...
          <div class="flex justify-center gap-4 mt-8">
            ...
            <div class="w-16 h-1 bg-[#009E60]"></div>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-7xl mx-auto px-4 py-12">
          ...
          <div v-for="(step, index) in steps" :key="index" class="flex-1 relative flex flex-col items-center">
            ...
            <div class="relative z-10">
              <div :class="[
                ...
                currentStep >= index + 1 ? 'bg-[#3A75C4] text-white' : 'bg-gray-200 text-gray-500',
                currentStep > index + 1 ? 'ring-4 ring-[#3A75C4]/30' : ''
              ]">
                {{ index + 1 }}
              </div>
            </div>
            <div class="text-xs mt-2 font-medium" :class="currentStep >= index + 1 ? 'text-[#3A75C4]' : 'text-gray-400'">
              {{ step.title }}
            </div>
            ...
            <div v-if="index < steps.length - 1"
                 ...
                 :class="currentStep > index + 1 ? 'bg-[#3A75C4]' : 'bg-gray-200'"
                 style="z-index: 1;">
            </div>
          </div>
        </div>
        ...
        <div class="p-8">
          ...
          <div v-show="currentStep === 1" class="space-y-6">
            <div class="border-b border-gray-200 pb-4">
              <h2 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                <svg class="w-6 h-6 text-[#3A75C4]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" v-model="form.fullname" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-[#3A75C4] focus:border-transparent transition" placeholder="Ex: Brice Boussougou">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Prénom(s) *</label>
                <input type="text" v-model="form.firstname" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-[#3A75C4] focus:border-transparent transition" placeholder="Prénom usuel">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Date de naissance *</label>
                <input type="date" v-model="form.dob" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-[#3A75C4] focus:border-transparent transition">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Lieu de naissance *</label>
                <input type="text" v-model="form.birthplace" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-[#3A75C4] focus:border-transparent transition" placeholder="Ville, Pays">
              </div>
              <div>
            ...
            <div class="flex justify-end pt-4">
              <button @click="nextStep" :disabled="!isFormValid" class="bg-[#3A75C4] text-white px-6 py-3 rounded-lg font-semibold hover:bg-[#2a5a9e] transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
                Suivant
          ...
          <div v-show="currentStep === 2" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="bg-gradient-to-r from-[#3A75C4]/10 to-[#009E60]/10 rounded-xl p-6 text-center">
                <svg class="w-12 h-12 text-[#3A75C4] mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                <h3 class="font-semibold text-gray-800 mb-2">Frais d'établissement</h3>
                <p class="text-3xl font-bold text-[#3A75C4] mb-2">50 USD</p>
            ...
            <div class="flex justify-between pt-4">
              ...
              <button @click="nextStep" :disabled="!allFilesValid" class="bg-[#3A75C4] text-white px-6 py-3 rounded-lg font-semibold hover:bg-[#2a5a9e] transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
                Suivant
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                </svg>
          ...
          <div v-show="currentStep === 3" class="space-y-6">
            ...
            <div class="bg-gray-50 rounded-xl p-6">
              <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-2xl font-bold text-[#3A75C4]">50 USD</span>
              </div>

              <div class="border-t border-gray-200 pt-4 mb-4">
                <label class="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" v-model="paymentConfirmed" class="w-5 h-5 text-[#3A75C4] rounded focus:ring-[#3A75C4]">
                  <span class="text-gray-700">Je confirme avoir lu les conditions et je m'engage à payer les 50 USD par carte bancaire</span>
                </label>
              </div>

              <button @click="simulatePayment" :disabled="!paymentConfirmed || paymentDone" class="w-full bg-[#009E60] text-white px-6 py-3 rounded-lg font-semibold hover:bg-[#006640] transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path>
                </svg>
            ...
            <div class="bg-red-50 border-l-4 border-[#3A75C4] p-4 rounded-r-xl">
              <div class="flex items-start gap-3">
                <svg class="w-5 h-5 text-[#3A75C4] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                </svg>
                <div>
                  <p class="font-bold text-gray-800">IMPORTANT :</p>
                  <p class="text-gray-700">Les documents susmentionnés devront être ensuite téléchargés sur la plate-forme après validation de votre demande.</p>
                </div>
              </div>
            </div>
            ...
            <div class="flex justify-between pt-4">
              ...
              <button @click="submitDossier" :disabled="!paymentDone" class="bg-[#3A75C4] text-white px-8 py-3 rounded-lg font-semibold hover:bg-[#2a5a9e] transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="submitted" class="bg-green-50 border border-green-200 rounded-2xl p-6 text-center">
        ...
        <p class="text-gray-600 mt-2">Un email de confirmation vous parviendra avec les instructions pour finaliser votre demande.</p>
      </div>
    </div>
  </div>
</template>
